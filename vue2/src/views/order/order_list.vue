<template>
  <div>
    <el-form v-show="showSearch" ref="queryForm" :model="queryParams" size="small" :inline="true" label-width="108px">
      ...
      <el-form-item v-if="!isShop" label="平台" prop="shopType">
    ...
    <el-table v-loading="loading" :data="orderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="商品" width="450px">
        <template slot-scope="scope">
          ...
          <template v-if="scope.row.itemList && scope.row.itemList.length">
            ...
            <div v-if="scope.row.itemList.length>1" style="float: left;display: flex;align-items: center;padding-left: 50px">
              <el-button
                size="mini"
                type="text"
                icon="el-icon-view"
      ...
      <el-table-column v-if="!isShop" label="店铺" align="left" prop="shopId" width="160px">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.shopType===0" size="small">销售订单</el-tag>
          <span v-else>
            <el-tag v-if="!isMerchant" size="small" style="padding-bottom: 10px;margin-bottom: 10px;" type="success">{{ merchantList.find(x=>x.id == scope.row.merchantId).name }}</el-tag>
      ...
      <el-table-column label="Tag" prop="isGift">
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.hasGift == -1" type="danger">赠品订单</el-tag>
          <el-tag v-if="scope.row.orderMode == 1" type="success">手工订单</el-tag>
        </template>
      </el-table-column>
          ...
          <el-form ref="form" :model="form" :rules="rules" label-width="80px" inline>
            <el-descriptions title="订单信息">
              ...
              <el-descriptions-item label="店铺">
                ...
                <el-tag v-if="form.shopType === 0" size="small">总部销售订单</el-tag>
                <el-tag v-if="form.shopType === 100" size="small">天猫</el-tag>
                <el-tag v-if="form.shopType === 300" size="small">拼多多</el-tag>
                <el-tag v-if="form.shopType === 400" size="small">抖店</el-tag>
                <el-tag v-if="form.shopType === 500" size="small">微信小店</el-tag>
                <el-tag v-if="form.shopType === 600" size="small">快手</el-tag>
                <el-tag v-if="form.shopType === 700" size="small">小红书</el-tag>
                <el-tag v-if="form.shopType === 200" size="small">京东POP</el-tag>
                <el-tag v-if="form.shopType === 280" size="small">京东自营</el-tag>
        ...
        <el-tab-pane label="商品列表" name="orderItems" lazy>
          <el-table :data="form.itemList" style="margin-bottom: 10px;">
            ...
            <el-table-column label="商品标题" prop="goodsTitle" width="260px">
              <template slot-scope="scope">
                {{ scope.row.goodsTitle }}
                <el-tag v-if="scope.row.isGift === 1" type="danger">赠品</el-tag>
              </template>
            </el-table-column>
